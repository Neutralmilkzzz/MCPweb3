...
### 🧪 重构：测试目录结构标准化 (分支: refactor/test-structure)

**⚠️ 注意：此变更会影响测试运行方式，但不影响生产代码**
...
- **新增功能**：
  - `tests/conftest.py`：自动 mock mcp 依赖，提供通用 fixtures
  - 自动标记：根据目录自动添加 `@pytest.mark.{category}`
  - 共享测试数据：`tests/fixtures/sample_responses.json`

- **工具更新**：
  - `run_tests.py` 完全重写，支持按类别运行、覆盖率、HTML 报告
  - `pyproject.toml` 新增 `[tool.pytest.ini_options]` 配置

- **测试结果**：全部 554 个测试通过 ✅

- **迁移影响**：
  - ✅ **生产代码零修改** - 仅测试文件移动
  - ✅ **Docker 镜像无影响** - `.dockerignore` 已排除 `tests/`
  - ⚠️ **CI/CD 需更新** - 将测试路径从 `test_*.py` 改为 `tests/`
  - ✅ **向后兼容** - `run_tests.py` 提供统一入口
